...
public static class IdentitySeeder
{
    ...
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        await EnsureOnboardingColumnsAsync(db);
        await EnsureAppointmentReminderSchemaAsync(db);
    ...
    private static async Task EnsureAppointmentReminderSchemaAsync(ApplicationDbContext db)
    {
        if (db.Database.IsSqlite())
        {
            var connection = db.Database.GetDbConnection();
            ...
            try
            {
                var profileColumns = await ReadSqliteColumnsAsync(connection, "PatientProfiles");
                await TryAddColumnAsync(db, profileColumns, "HomeAddress",
                    """ALTER TABLE "PatientProfiles" ADD COLUMN "HomeAddress" TEXT NULL""");
                await TryAddColumnAsync(db, profileColumns, "NamePronunciation",
                    """ALTER TABLE "PatientProfiles" ADD COLUMN "NamePronunciation" TEXT NULL""");
                await TryAddColumnAsync(db, profileColumns, "AppointmentRemindersEnabled",
                    """ALTER TABLE "PatientProfiles" ADD COLUMN "AppointmentRemindersEnabled" INTEGER NOT NULL DEFAULT 1""");

                var appointmentColumns = await ReadSqliteColumnsAsync(connection, "Appointments");
                await TryAddColumnAsync(db, appointmentColumns, "Purpose",
                    """ALTER TABLE "Appointments" ADD COLUMN "Purpose" TEXT NULL""");

                await db.Database.ExecuteSqlRawAsync(
                    """
                    CREATE TABLE IF NOT EXISTS "AppointmentReminders" (
                        "Id" INTEGER NOT NULL CONSTRAINT "PK_AppointmentReminders" PRIMARY KEY AUTOINCREMENT,
                        "AppointmentId" INTEGER NOT NULL,
                        "Kind" INTEGER NOT NULL,
                        "Channel" INTEGER NOT NULL,
                        "StartsAtSnapshot" TEXT NOT NULL,
                        ...
                        "Detail" TEXT NULL,
                        CONSTRAINT "FK_AppointmentReminders_Appointments_AppointmentId" FOREIGN KEY ("AppointmentId") REFERENCES "Appointments" ("Id") ON DELETE CASCADE
                    );
                    """);
                await db.Database.ExecuteSqlRawAsync(
                    """
                    CREATE UNIQUE INDEX IF NOT EXISTS "IX_AppointmentReminders_AppointmentId_Kind_Channel_StartsAtSnapshot"
                    ON "AppointmentReminders" ("AppointmentId", "Kind", "Channel", "StartsAtSnapshot");
                    """);
            }
            finally
            {
        ...
        await db.Database.ExecuteSqlRawAsync(
            """ALTER TABLE "PatientProfiles" ADD COLUMN IF NOT EXISTS "HomeAddress" text NULL;""");
        await db.Database.ExecuteSqlRawAsync(
            """ALTER TABLE "PatientProfiles" ADD COLUMN IF NOT EXISTS "NamePronunciation" text NULL;""");
        await db.Database.ExecuteSqlRawAsync(
            """ALTER TABLE "PatientProfiles" ADD COLUMN IF NOT EXISTS "AppointmentRemindersEnabled" boolean NOT NULL DEFAULT TRUE;""");
        await db.Database.ExecuteSqlRawAsync(
            """ALTER TABLE "Appointments" ADD COLUMN IF NOT EXISTS "Purpose" text NULL;""");
        await db.Database.ExecuteSqlRawAsync(
            """
            CREATE TABLE IF NOT EXISTS "AppointmentReminders" (
                ...
                "Kind" integer NOT NULL,
                "Channel" integer NOT NULL,
                "StartsAtSnapshot" timestamp without time zone NOT NULL,
                "Status" integer NOT NULL,
                "AttemptCount" integer NOT NULL,
    ...
    private static async Task<HashSet<string>> ReadSqliteColumnsAsync(System.Data.Common.DbConnection connection, string table)
    {
        var columns = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        await using var cmd = connection.CreateCommand();
